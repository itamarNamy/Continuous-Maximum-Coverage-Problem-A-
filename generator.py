...
def random_convex_polygon(n_points=5, center=(0, 0), radius=1):
    """
...
def generate_non_intersecting_polygons(n_polygons=100, n_points=5, radius=5):
    polygons = []
    for i in range(n_polygons):
        while True:
            center = (random.uniform(-10, 10), random.uniform(-10, 10))  # Generate random center point
            polygon = random_convex_polygon(n_points=n_points, center=center, radius=radius)
